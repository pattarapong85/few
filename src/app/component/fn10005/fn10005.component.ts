...
@Component({
  selector: 'app-fn10005',
  templateUrl: './fn10005.component.html',
  styleUrls: ['./fn10005.component.scss']
})
export class Fn10005Component implements OnInit {
  ...
  this.cols = [
      { field: 'CD', header: 'รหัสหมวดหมู่' },
      { field: 'VALUE', header: 'ชื่อหมวดหมู่' },
  ...
  validateBeforeSave(newRow : Dropdown){
    if(newRow.CD != undefined && newRow.CD.trim().length == 0) {
      ...
      return false;
    }else if(this.checkDuplicate(newRow.CD)){
...
save() {
  ...
  if (this.validateBeforeSave(this.newRow)) {
    ...
    this.newRow.SUB_CATEGORY = this.app.PRODUCT.SUB_CATEGORY_CATEGORY_PRODUCT;
